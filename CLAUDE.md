...
- 쓰기 상위 10개 합계 **474,322행**(2026-08-29, `0027` 배포 후 만 하루가 지난 깨끗한 창) —
  **한도의 4.7배다. 여기는 아직 미해결이다.** `realtime_parking_status` upsert 하나가
  343,553행(72%)이고, `discovery_items` upsert는 `0027` 인덱스 정리로 158,788 → 97,494행이 됐다.
  인덱스 정리로 줄일 수 있는 몫은 사실상 다 썼고, 남은 건 realtime 쪽 동작을 바꾸는 것뿐이다.
...
- **다만 그 값은 상한이지 항상은 아니다.** `ON CONFLICT DO UPDATE`는 `SET`에 등장해 실제로
  ...
  빼는 것만으로도 쓰기가 준다 — `lat`/`lng`를 빼면 하루 114,515행이 줄고, 대신 원본의 좌표
  수정이 기존 행에 반영되지 않는다.
