...
import { createCulturePortalResolver, CulturePortalEventProvider } from "./CulturePortalEventProvider.js";
...
export function createEventService(): EventService {
  ...
  const culturePortalKey = config.CULTURE_PORTAL_API_KEY ?? config.PUBLIC_DATA_SERVICE_KEY;
  if (culturePortalKey) {
    providers.push(new CulturePortalEventProvider(culturePortalKey, config.PUBLIC_DATA_BASE_URL, resolver));
  }
  if (config.KOPIS_API_KEY) {
    providers.push(new KopisEventProvider(config.KOPIS_API_KEY, config.KOPIS_BASE_URL, resolver));
  }
...
function dedupeEvents(items: FreeEvent[]): FreeEvent[] {
  const selected = dedupeCachedEvents(items.map((item) => ({
    id: item.id,
    title: item.title,
  ...
  return selected.map((item) => ({
    ...item,
    status: items.find((candidate) => candidate.id === item.id)?.status ?? "upcoming",
    distanceMeters: items.find((candidate) => candidate.id === item.id)?.distanceMeters ?? 0
  }));
}
