...
const REGION_FALLBACKS: Array<[RegExp, { lat: number; lng: number }]> = [
  [
...
// 좌표를 못 구한 항목이 지역 대표 좌표에 그대로 쌓이면 지도에서 한 점에 수백 개가 겹치므로,
// 응답 단계에서 이 좌표들을 걸러낼 수 있도록 목록을 함께 내보낸다.
export const REGION_FALLBACK_COORDINATES: ReadonlyArray<{ lat: number; lng: number }> =
  REGION_FALLBACKS.map(([, coordinate]) => coordinate);
...
export function regionFallbackCoordinate(
  ...
  if (!text) return null;
  return REGION_FALLBACKS.find(([pattern]) => pattern.test(text))?.[1] ?? null;
}
