...
## 2026-05-09 direction update: parking plus local discovery
...
3. Festival and event layers remain the discovery expansion beside parking.
4. The bottom discovery list shows festivals and events with search and sorting.
5. Opening a festival or event on the map recenters the app on that item and reloads nearby parking/realtime context.

### Preserve the current plan

- Keep the core `destination search -> parking recommendations -> detail/navigation` flow intact.
- Treat festivals, events, and realtime parking as additive layers, not replacements for parking.
