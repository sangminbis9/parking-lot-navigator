...
## 2026-09-13: 행사 조회를 일일 스냅샷 + 로컬 조회로 전환

- 배경: D1 한도 초과로 지도 핀이 사라졌고, 응답 페이지네이션만으로는 DB 읽기 비용이 줄지 않았다.
- 결정: 행사 공개 DTO를 하루 한 번 R2/CDN에 발행하고 앱은 완성본을 저장해 지도·목록·달력을 로컬 조회한다. 주차 잔여면은 별도 실시간 경로다.
- 실패 정책: 중간/검증 실패 파일을 활성화하지 않고 기존 완성본 유지. 무거운 D1 탐색 API로 자동 fallback하지 않는다. 서버 먼저 배포하고 앱을 배포한다.
- 당일 변경: 관리자 재발행 + 앱 15분 버전 확인. 사용자가 받지 못한 변경을 오프라인에서 보장하지 않는다.
