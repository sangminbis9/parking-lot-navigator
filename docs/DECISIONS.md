...
## 2026-09-16: 사장님 직접 게시 종료는 기록 보존형 비노출로 처리

- 게시 중인 이벤트만 사장님이 직접 내릴 수 있으며, 경고 팝업의 명시적 확인을 거쳐야 한다. 소유권과 현재 상태는 서버에서 다시 검증한다.
- 행을 삭제하지 않고 기존 status 값 `expired`와 `rejection_reason=merchant_withdrawn` 조합을 사용한다. 결제·게시 기간·이미지를 거래 기록으로 보존하고 D1 스키마 마이그레이션을 피한다.
- 사장님 요청으로 이미 시작된 게시를 조기 종료하면 잔여 기간 부분 환불은 제공하지 않는다. 단, 관련 법령상 의무와 회사 귀책 환불은 기존 환불·취소 정책에 따라 예외로 둔다.

## 2026-09-15: Slack 일일 운영 통계는 익명·저비용 집계로 발송

- 매일 20:00 KST Slack Incoming Webhook으로 접속자, 신규 축제/박람회/공연/로컬 이벤트 수와 사장님 직접 등록 카드 상세를 보낸다.
- 접속자는 서버에 기기 식별자를 저장해 distinct하지 않는다. iOS가 설치별 KST 하루 한 번만 기존 `app_open` 카운터를 올려 best-effort DAU로 사용한다. 개인정보 최소화와 D1 쓰기 절감을 정확한 계정 기반 DAU보다 우선한다.
- 20:10/20:20 재시도와 R2 완료표시를 사용한다. 별도 Queue를 쓰지 않아 이미 빠듯한 Queue 일일 예산을 소비하지 않는다.
- 일일 1회 소규모 full scan이 수집 write amplification보다 싸므로 신규 D1 인덱스/마이그레이션은 만들지 않는다. Slack URL은 Worker secret으로만 관리한다. [상세 설계](architecture/daily-slack-report.md).

## 2026-09-13: 행사 조회를 변경 기반 스냅샷 + 로컬 조회로 전환

- 배경: D1 한도 초과로 지도 핀이 사라졌고, 응답 페이지네이션만으로는 DB 읽기 비용이 줄지 않았다.
- 결정: 최초 전체 공개 DTO를 R2에 발행한 뒤, 기존 수집·보강·사장님 등록에서 실제 변경된 고정 묶음만 다시 발행한다. 초기 일일 전체 발행 계획은 폐기했다. 앱은 완성본을 저장해 지도·목록·달력을 로컬 조회한다.
- 실패 정책: 중간/검증 실패 파일을 활성화하지 않고 기존 완성본 유지. 무거운 D1 탐색 API로 자동 fallback하지 않는다. 서버 먼저 배포하고 앱을 배포한다.
- 당일 변경: DB 트리거로 변경을 원자적으로 기록, 매분 복구 검사, 일반 행사 2분 합치기/사장님 행사 우선 처리. 앱 활성 화면에서 60초 버전 확인. 예산 소진·오프라인에서는 즉시 반영을 보장하지 않는다.
- 실시간 주차는 기존 수집 주기로 R2 공유 캐시를 발행하고 활성 지도에서 현재 영역을 45초마다 확인한다. R2 발행과 legacy D1 쓰기 오류를 격리한다. Worker 파일 전달 주소를 우선 사용하며, 직접 CDN·유료 플랜 전환은 별도 결정이다.
- 초기 구현은 전국 분할 파일 완전 다운로드다. 운영 크기/메모리 실측 후 필요하면 지역별 팩으로 확장한다. [상세](architecture/discovery-snapshots.md).
