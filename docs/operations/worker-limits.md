...
| invocation당 CPU 시간 | 10ms | 예외 없이 isolate가 종료된다. `wrangler tail`에 `Exceeded CPU Limit`으로만 남고 `try/catch`도 `notifyOpsFailure`도 타지 않는다. 진행 중이던 D1 쓰기는 손실. | cron 핸들러 전부. 특히 태깅 + backfill이 한 invocation에 얹힌 `*/20` |
...
- **배치 쓰기를 회차 끝에 몰지 않는다.** 루프를 다 돌고 나서 `db.batch()`를 한 번만
  ...
  지금은 `DISCOVERY_FLUSH_SIZE`(5건)마다 나눠 쓴다.
...
올라간다. 지금 파이프라인 손실의 최대 단일 원인이 CPU 한도이므로, 배치 크기를 손대기
전에 이 전환부터 검토한다. 전환하면 다음이 함께 풀린다.

- `LOCAL_EVENT_MAX_KAKAO_LOOKUPS`를 올려 로컬 이벤트 커버리지 확대
- `*/20`의 fee/geocode/image 3분할 로테이션을 없애고 매 회차 전부 실행
