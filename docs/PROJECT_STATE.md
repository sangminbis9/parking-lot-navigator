...
- 첫 완성 데이터 발행·전체 검증 완료: `9f2f03cd-e632-42e2-9696-a2a63f8cee12`, 공개 항목 12,669개 / 167파일 / 11,500,812바이트. 미처리 변경 조회 0개, 내부 catalog/build/예산 경로 모두 HTTP 404. 최신 Worker `3a195f69-c270-4756-9b17-2a88caea4548`.
- 최종 로컬 검증: Worker 39파일/338테스트, 공통 backend 23파일/73테스트, TypeScript 통과. iOS 검증은 실제 공개 데이터로 수행하도록 강화했고 상세 목록 부재 시 skip 대신 실패한다. 새 iOS/TestFlight는 아직 완료하지 않았다.

- 일일 전체 발행을 고정 묶음 변경 추적으로 전환. D1 0032 원격 적용, Worker `6890b1c5-e3d9-4a62-92e1-565e009ae939` 배포 완료. 09:22 실제 Worker 행 읽기/묶음 발행 전진 확인. 최초 완성 manifest는 검증 중이다.
- 일반 행사 변경 합치기 2분, 지역/사장님 행사 우선 처리, 미처리 변경 매분 복구. Queue 신규 발행 메시지 700/UTC일 예산 및 한도 초과 대기. 삭제/비공개·중복/동시 변경·실패 시 이전 데이터 보존.
- 실시간 주차 조회는 R2 공유 캐시로 전환. iOS는 활성 관련 화면에서 행사 버전 60초, 지도 주차 레이어는 현재 영역 45초 확인. iOS 변경은 아직 검증·TestFlight 미완료.
- Worker 전체 330개 및 신규 실시간 4개 테스트, TypeScript, 로컬 전체 D1 마이그레이션 검증 완료. 배포 workflow의 강제 수집 워밍업/넓은 D1 smoke 제거. 세부 계약과 비용은 [운영 문서](architecture/discovery-snapshots.md) 참조. 아래 일일 설계 기록은 이전 이력이다.
