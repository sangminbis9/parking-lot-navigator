...
- Widget Extension: `com.sangminbis9.ParkingLotNavigator.UpcomingFestivalsWidget`
- App Group: `group.com.sangminbis9.ParkingLotNavigator`

App Intents는 첫 빌드 안정성을 위해 메인 앱 target에 포함합니다. Share Extension / Widget Extension target은 모두 같은 App Group entitlement를 사용할 수 있게 설정합니다.

세 App ID **모두** App Groups capability 에서 **Configure 버튼**으로 위 그룹 ID 를 명시 매핑해야 합니다. 체크박스만 켜고 Configure 를 누르지 않으면 매핑이 비어 있는 채로 저장되며, Codemagic 빌드가 `requires a provisioning profile with the App Groups feature` 오류로 실패합니다.
...
- `requires a provisioning profile with the App Groups feature` (extension target): App ID 에 App Groups capability 가 켜져 있어도, **Configure 버튼으로 그룹 ID 가 명시 매핑되어 있지 않으면** 발생합니다. 매핑 수정 후 새 provisioning profile 을 발급해 다시 업로드해야 합니다.
...
- `ValidateEmbeddedBinary ... appex`: extension target 의 `PRODUCT_BUNDLE_IDENTIFIER` 가 비었거나 메인 앱 Bundle ID 의 prefix 가 아닌 경우. project.yml 에서 `$(APP_BUNDLE_ID).XXX` 형태로 inline 파생하면 안전합니다 (별도 xcconfig 변수 누락 사고를 회피).

## 7. App Extension 추가 시 체크리스트

새 app extension target (위젯, intent extension 등) 을 추가할 때 다음 순서를 그대로 따르면 사이닝/임베드 오류를 피할 수 있습니다.

1. Apple Developer Portal 에서 새 App ID 등록 — 예) `com.sangminbis9.ParkingLotNavigator.UpcomingFestivalsWidget`.
2. 그 App ID 의 Capabilities 에서 App Groups 체크 → **Configure** 버튼 → `group.com.sangminbis9.ParkingLotNavigator` 매핑 후 Save.
3. Profiles 메뉴에서 메인 앱과 동일한 type (App Store / Ad Hoc / Development) + 동일 distribution certificate 로 새 provisioning profile 발급, `.mobileprovision` 다운로드.
4. Codemagic 대시보드 → App settings → Code signing identities → **Provisioning profiles** 에 새 파일 업로드. 수동 사이닝이면 Bundle ID 매칭으로 자동 선택됨.
5. `ios-app/project.yml` 의 새 target `PRODUCT_BUNDLE_IDENTIFIER` 를 `$(APP_BUNDLE_ID).XXX` 형식으로 작성하고, 메인 target 의 `dependencies` 에 새 target 을 추가.
6. `CURRENT_PROJECT_VERSION` 을 +1 한 뒤 커밋·푸시 → Codemagic 빌드 재시작.
