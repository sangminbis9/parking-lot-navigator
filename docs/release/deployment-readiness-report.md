...
1. ✅ 해결 — `PrivacyInfo.xcprivacy` 존재. 단 CrashData 수집을 선언했으나 크래시 SDK 미연동이라 정합성 확인 필요
2. 🟡 코드 완료 — 개인정보처리방침/이용약관/환불정책을 Worker `/legal/*` 라우트로 구현. 남은 일: 배포 + 공개 URL을 App Store Connect에 입력(사용자), 보호책임자란 사업자등록 후 갱신
3. 🔴 iOS 크래시 트래킹(Sentry/Crashlytics) 미연동 — 라이브 운영 불가 수준
4. 🔴 Worker Cron 실패 알림 부재 — realtime/sync cron 실패 무중단 감지 불가(6장)
5. 🔴 App Store 스크린샷 5장 + Privacy 질문지 답변 미작성 — 제출 자체 차단(사용자/디자인)
...
| 항목                                       | 현재 상태                                          | 우선순위          | 권장 조치                                                                                                                                                                                                  |
| ------------------------------------------ | -------------------------------------------------- | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PrivacyInfo.xcprivacy` (Privacy Manifest) | 있음 (`ios-app/Resources/PrivacyInfo.xcprivacy`, 84줄) | 🟢 OK | 해결됨 — PreciseLocation·CrashData 등 선언. ⚠️ CrashData를 선언했으나 크래시 SDK 미연동(6장) → SDK 추가하거나 선언 정리로 정합성 확보 |
...
| Codemagic 빌드 번호 자동 증가 | 수동 (`CURRENT_PROJECT_VERSION` 160)                                       | 🟠 P2    | `CI_BUILD_NUMBER` 또는 codemagic.yaml `agvtool` 자동                               |
...
| #   | 항목                                         | 차단 사유                   | 1차 산출물                                                |
| --- | -------------------------------------------- | --------------------------- | --------------------------------------------------------- |
| 1 | `PrivacyInfo.xcprivacy` | ✅ 해결 — 파일 존재 | CrashData 선언 vs 크래시 SDK 미연동 정합성만 확인 |
...
| 4 | iOS 크래시 트래킹 (Sentry/Crashlytics) | 🔴 미해결 | Sentry SPM 추가 (PrivacyInfo가 이미 CrashData 선언) |
| 5 | App Store 스크린샷 5장 + Privacy 질문지 답변 | 🔴 미해결 | 6.7"/6.5"/5.5" 스크린샷 + Privacy questionnaire 초안 (사용자/디자인) |
