...
차단 위험 항목 (2026-05-31 재검사 반영):

1. ✅ 해결 — `PrivacyInfo.xcprivacy` 존재 + 1st-party 정합성 정리(CrashData/Performance 선언 제거, 크래시 SDK 미연동과 일치)
2. 🟡 코드 완료 — 개인정보처리방침/이용약관/환불정책을 Worker `/legal/*` 라우트로 구현. 남은 일: **배포**(인증 토큰 필요, 사용자 실행) + 공개 URL을 App Store Connect에 입력(사용자), 보호책임자란 사업자등록 후 갱신
3. 🔴 iOS 크래시 트래킹(Sentry/Crashlytics) 미연동 — 라이브 운영 불가 수준. (PrivacyInfo는 미수집으로 정합성 맞춤)
...
| App Store Privacy 질문지 답변 | 초안 존재 (`docs/release/app-store-privacy-answers.md`, 122줄) | 🟡 P1 | 위치/진단/구매기록(Toss)/식별자/User Content 항목별 답변 초안 작성됨. 남은 일: ASC에 실제 입력(사용자). 단, 진단(Crash/Performance) 항목은 SDK 미수집이면 ASC에서도 No로 맞춰야 함 |
| 개인정보처리방침 호스팅 URL | Worker `/legal/privacy` 라우트 구현됨(실콘텐츠) | 🟡 P1 | 코드 완료. 배포 후 공개 URL을 App Store Connect에 입력 필요(사용자 확인). 보호책임자란은 사업자등록 후 갱신 예정 |
| 이용약관 / 환불·취소 정책 | Worker `/legal/terms`·`/legal/refund-policy` 구현됨 | 🟡 P1 | 코드 완료(전상법 항목 포함). 배포·앱 내 노출 연결 확인 필요(사용자) |
...
| Kakao Maps SDK · 공공데이터 출처 표기 | Settings에 "Kakao Local, 서울 열린데이터광장, data.go.kr" 표기 존재 | 🟡 P1 | 부분 해결. KOPIS·TourAPI 출처와 "© Kakao Map" 지도 저작권 표기 보강 필요 |
...
| 데이터 출처 표기 | Settings에 일부 표기(data.go.kr/서울 열린데이터/Kakao) | 🟡 P1 | KOPIS·TourAPI 누락분 추가, 지도 저작권 표기 |
...
| Cron 실패 알림           | 코드 구현됨 (`notifyOpsFailure`, 6개 cron catch) | 🟡 P1    | webhook 알림 코드 완료(Discord/Slack 자동 분기). 활성화: Worker 배포 + `OPS_ALERT_WEBHOOK_URL` secret 설정. ⚠️ realtime(1분) cron이 지속 실패 시 알림 빈발 가능 — 필요 시 throttle 추가 |
...
| #   | 항목                                         | 차단 사유                   | 1차 산출물                                                |
| --- | -------------------------------------------- | --------------------------- | --------------------------------------------------------- |
| 1 | `PrivacyInfo.xcprivacy` | ✅ 해결 — 파일 존재 + 1st-party 정합성 정리 | 완료 (CrashData/Performance 선언 제거) |
| 2 | 개인정보처리방침 호스팅 URL | 🟡 코드 완료 — Worker `/legal/privacy` | 배포 후 ASC에 URL 입력(사용자) |
| 3 | 이용약관 + 환불·취소 정책 | 🟡 코드 완료 — `/legal/terms`, `/legal/refund-policy` | 배포·앱 내 노출 연결 |
| 4 | iOS 크래시 트래킹 (Sentry/Crashlytics) | 🔴 미해결 | Sentry SPM 추가(DSN 발급+iOS 빌드 필요). 미도입 시 PrivacyInfo는 이미 미수집으로 정합 |
| 5 | App Store 스크린샷 5장 | 🔴 미해결 | 6.7"/6.5"/5.5" 스크린샷 (사용자/디자인). Privacy 질문지는 초안 존재 → ASC 입력만 남음 |
