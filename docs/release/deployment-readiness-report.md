...
| 항목                                | 현재 상태                                | 우선순위 | 권장 조치                                                                                     |
| ----------------------------------- | ---------------------------------------- | -------- | --------------------------------------------------------------------------------------------- |
| 앱 스토어 스크린샷 (6.7"/6.5") | 없음 — `docs/release/`에 이미지 산출물 없음(2026-09-11 재확인) | 🔴 P0    | **제출을 막는 유일한 코드 외 산출물.** 5장 이상, 첫 2장이 결정적(지도 + 행사 상세 + 캘린더). iOS 17 이후 6.7"와 6.5" 중 하나면 되지만 ASC 요구 사항을 제출 시점에 확인할 것 |
...
| 5 | App Store 스크린샷 | 🔴 **P0 — 유일하게 남은 차단 요소** | 필수 규격은 **6.9" 1장 이상**(1320×2868 / 1290×2796 / 1260×2736, sRGB, 투명도 불가)이고 6.5"는 6.9"를 내지 않을 때만 필요하다 (2026-09-11 확인 — 이전 판의 `6.7"/6.5" 5장` 서술은 현재 요구사항과 다르다). 지도 + 행사 상세 + 캘린더가 앞쪽에 들어가야 한다(사용자/디자인). 로컬 Mac이 없으므로 `.github/workflows/ios-screenshots.yml`을 수동 실행해 macOS 러너 시뮬레이터에서 캡처한다 |
