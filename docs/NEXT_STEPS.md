...
- 브랜치: `master`
- 마지막으로 push 된 커밋: `3e64067 Add customizable festival/local-event notification settings with background discovery`
...
- iOS 빌드 번호는 `ios-app/project.yml` 에서 `1.0 (164)` 이다. 알림 설정 기능 변경분이라 다음 Codemagic 빌드로 컴파일 확인이 필요하다(이 번호의 빌드는 아직 미검증). 성공 빌드 확인 후 다음 TestFlight 업로드 시 `≥ 165` 로 bump 한다.
...
- 마지막 성공 빌드 번호는 `1.0 (134)` (`f3465f2` + `c65a3a5` 반영 후). 다음 Codemagic/TestFlight 업로드 시 `CURRENT_PROJECT_VERSION` 을 `≥ 135` 로 bump 한 뒤 publish 로그의 `Version code` 가 새 값을 가리키는지 확인한다.
