...
const EVENT_PHRASE_PATTERN =
  /(오픈\s*이벤트|리뷰\s*이벤트|방문\s*이벤트|체험단|할인\s*이벤트|할인\s*행사|1\s*\+\s*1|무료\s*증정|쿠폰\s*증정|오픈\s*기념|오픈\s*특가|신메뉴\s*출시|신메뉴|한정\s*판매|한정\s*메뉴|선착순|이벤트\s*중|이벤트\s*진행|그랜드\s*오픈|리뉴얼\s*오픈|팝업\s*스토어|시식\s*회|시식\s*행사|오픈\s*행사|기간\s*한정|시즌\s*메뉴|단독\s*이벤트)/g;
