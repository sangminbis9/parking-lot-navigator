...
<body>
  ...
  <ul>
    ...
    <li>알림 수신 정보: 알림을 켠 경우에만 APNs 기기 토큰, 앱이 생성한 익명 기기 식별자, 사용자가 고른 관심 지역·카테고리·방해 금지 시간. 다가오는 행사 알림 발송 목적으로만 사용합니다.</li>
    <li>익명 사용 통계: 화면 열람·핀 선택 같은 정해진 동작의 <strong>횟수</strong>만 집계합니다. 기기 식별자, 세션 식별자, 좌표, 검색어를 함께 보내지 않으며 개인을 식별하거나 광고에 사용하지 않습니다.</li>
    <li>행사 정보 오류 신고: 신고 대상 행사 식별자, 신고 사유, 사용자가 직접 입력한 메모(선택). 신고자를 식별하는 정보는 저장하지 않습니다.</li>
  </ul>
  ...
  <ul>
    ...
    <li>알림 수신 정보: 앱에서 알림을 모두 끄거나 앱을 삭제해 토큰이 무효화되면 더 이상 발송에 사용하지 않으며, 삭제를 원하면 아래 문의처로 요청할 수 있습니다. 발송 이력은 30일 후 자동 삭제됩니다.</li>
    <li>익명 사용 통계: 날짜별 합계 형태로만 보관하며 개별 이용자의 행동 이력으로 복원할 수 없습니다.</li>
  </ul>
...
<body>
  ...
  <p class="meta">시행일: 2026-05-25 · 최종 개정: 2026-09-16 · 적용 대상: 이벤트다 가맹점 이벤트 게시 상품</p>
  ...
  <h2>3. 부분 환불 기준</h2>
  <ul>
    <li>가맹점이 이미 게시 중인 이벤트를 직접 조기 종료하는 경우: 남은 게시 기간에 대한 부분 환불은 제공하지 않습니다. 단, 법령상 환불 의무가 있거나 회사 귀책 사유가 있는 경우는 제외합니다.</li>
