...
// wave 22: 경기도에서 지금까지 한 번도 조사되지 않았던 4곳(여주시, 연천군,
// 가평군, 양평군)을 처음부터 조사한 wave다(2026-08-01 실측). 결론부터: 4곳
// 모두 등록 0건이다 — 시청 본 도메인은 대부분 robots.txt로 전면 차단됐고,
// 대안으로 찾은 문화재단/문화원/시설관리공단 도메인은 robots는 통과해도
// 실제 게시판이 "등록일(작성일)만 있는 게시판", "여러 단일 행사 마이크로
// 사이트의 모음", 또는 "축제가 아닌 콘텐츠(개별 공연·행정 공지·홍보성
// 캠페인)가 섞여 서버사이드로 분리할 수 없는 게시판"이라 이전 wave들의
// 배제 기준에 정확히 걸렸다.
//
// - 여주시: 시청 본 도메인(www.yeoju.go.kr) robots.txt는 `User-agent: *`가
//   `Disallow: /`이면서 `Allow: /tour/`, `Allow: /alddeul/`만 예외라 관광
//   섹션(/tour/)만 접근 가능하다(2026-08-01 확인). /tour/index.do의
//   "축제·행사" 메뉴를 열어보니 한글날 경축행사/여주도자기축제/
//   여주오곡나루축제/여주흥천남한강 벚꽃축제/여주금사참외축제 5개가 각각
//   `/tour/contents.do?key=976~980`처럼 축제 하나씩을 소개하는 독립
//   정적 페이지로 링크돼 있을 뿐, 한 페이지 안에 여러 축제가 카드로
//   반복되는 목록 구조가 아니다 — "여러 개의 단일 행사 소개 페이지 모음"
//   이라 declarative parser로 스캔할 단일 리스트가 없어 제외.
//   대안으로 wave 21에서 "여주+세종 문화재단"으로 확인됐던 yjcf.or.kr(여주
//   세종문화관광재단)을 이번엔 여주시 후보로 정식 재검토했다. robots.txt는
//   `User-agent: *` / `Allow: /`로 전면 허용이다(www/non-www 동일,
//   2026-08-01 확인). GNB "공연·축제·행사 > 공연안내 > 공연일정"
//   (`/reserve/board/1/M/L/menu/401`)은 실제 반복형 목록
//   (`ul.perform_list > li`, `.per_tit` 제목/`.per_day`
//   "일시 : 2026.08.26~2026.08.26" 형식의 연도 포함 기간/`.per_place` 장소)
//   이 있어 구조적으로는 통과하지만, 확인한 3건 전부가 "[재즈] 8월 문화가
//   있는 날 <살롱 드 쿠바>", "[뮤지컬] 8월 기획공연 <어린이 캣>", "떼창데이
//   VOL.2 <김경호 X 포지션 콘서트>"처럼 세종국악당 단일 공연장의 개별
//   콘서트/뮤지컬 프로그램이었다 — "축제"가 아니라 이 프로젝트에서
//   KOPIS(`/api/performances`)가 별도로 담당하는 "공연" 도메인에 해당해
//   제외 대상이다. 같은 GNB의 "행사안내 > 행사일정"
//   (`/edu/board/2/M/menu/262`)도 같은 구조의 게시판이지만 확인한 5건 중
//   진짜 축제성 항목("여주 남한강 출렁다리 별빛극장")은 1건뿐이고 나머지는
//   "여주관광상품권 SNS 이용후기 이벤트", "여주 군고구마빵 '여고빵'",
//   "여주마블 스탬프투어"처럼 관광 프로모션 캠페인이었다 — 세종시
//   sjcf.or.kr(wave 19)와 동일하게 서버사이드로 "진짜 축제"만 분리할 카테고리
//   필터가 없어(검색 select는 `searchField`=제목/내용 검색만 지원) 제외.
//   상세 링크도 `onclick="javascript:proc('523')"` 형태의 JS 호출이라
//   href로 detailUrl을 못 얻어 어차피 customParser가 필요했던 점도 참고.
//   결과적으로 여주시는 등록 가능한 사이트를 찾지 못했다.
//
// - 연천군: 시청 본 도메인(yeoncheon.go.kr, www 포함)과 관광
//   서브도메인(tour.yeoncheon.go.kr)은 robots.txt가 완전히 동일하다 —
//   `User-agent: Googlebot` 그룹에는 `/festival/`, `/tour/` 등 다수 경로에
//   `Allow`가 있지만, `User-agent: *` 그룹은 `Disallow: /` 단독이다
//   (2026-08-01 확인). 우리 UA(`Mozilla/5.0 ParkingLotNavigator/1.0`)는
//   "Googlebot"과 정확히 일치하지 않아 `*` 그룹만 적용되므로 도메인
//   자체가 전면 차단이다 — "연천 구석기 겨울여행축제" 전용
//   `yeoncheon.go.kr/festival/index.do`도 이 규칙에 걸려 애초에 접근
//   대상이 아니다. 대안으로 연천문화원(검색으로 찾은 실제 도메인
//   `xn--z92b19qita13pqrn.kr`, 연천문화원.kr의 퓨니코드)을 확인했으나
//   robots.txt가 `User-agent: Yeti`/`User-agent: Googlebot`은 `Allow: /`,
//   `User-agent: *`는 `Disallow: /`라 동일한 이유로 전면 차단이다
//   (2026-08-01 확인). 연천군시설관리공단(yccs.or.kr)은 robots.txt가
//   `User-agent: *` 그룹에 `Allow: /`이고 문의/FAQ 등 일부 경로만
//   `Disallow`라 허용되지만(2026-08-01 확인, http는 403을 반환하나 https는
//   200으로 정상 응답), 유일한 후보 게시판인 "알림마당"(`/pub/event.do`)이
//   `dzSmart/board/js/dzSmartBoard.js`로 클라이언트 사이드에서 항목을
//   그려내는 구조라 raw HTML curl에는 목록 카드가 전혀 나타나지 않았다(홈
//   화면에 노출된 "연천수레울아트홀 7월~9월 공연안내" 등 5개 항목의 ID가
//   `/pub/event.do` 응답 본문에 0건 매치, 2026-08-01 확인) — 게다가 그
//   5개 항목도 제목 자체가 "연천수레울아트홀 ○월 공연안내"처럼 단일
//   문화시설의 공연 프로그램 안내라 애초에 "축제" 신호가 약하다. 두
//   사유(JS 렌더링으로 declarative parser 불가 + 축제 도메인 신호 부족)
//   모두에 해당해 제외. 결과적으로 연천군은 시청 도메인이 robots로 전면
//   차단되고, 찾은 대안 2곳도 각각 robots 차단과 JS 렌더링/도메인 불일치로
//   제외돼 등록 가능한 사이트를 찾지 못했다.
//
// - 가평군: 시청 본 도메인(gp.go.kr, www 포함)의 robots.txt는
//   `User-agent: Googlebot` 그룹(`/ksf/search.jsp`, `/*/selectBbs` 경로만
//   Disallow)만 있고 `User-agent: *` 그룹 자체가 없다(2026-08-01 확인) —
//   이전 wave들의 해석(부산 중구 등)대로 우리 UA는 어느 그룹과도 매치되지
//   않아 비차단으로 판단했다. 실제 콘텐츠는 `www.gp.go.kr/portal/index.do`
//   리다이렉트 뒤에 있고, GNB의 "문화축제교육행사"
//   (`/portal/selectBbsNttList.do?bbsNo=76&key=443`) 게시판을 확인했다 —
//   구조는 반복형 테이블(`table.bbs_default_list`)이지만 컬럼이
//   번호/제목/부서/**작성일**/조회수뿐이고 실제 행사 일정은 어디에도 없다
//   (예: "스크린 화면으로 찾아오는 뮤지컬!! 상영작 안내" 항목의 날짜는
//   게시일 "2026-07-20" 하나뿐). 게다가 최근 10건 표본에 "제5회 경기북부
//   안심치안포럼 개최" 같은 행정 공지까지 섞여 있어 "등록일만 있는
//   게시판" + "축제 신호 없는 행정 게시판" 두 배제 기준에 모두 해당해
//   제외. 관광 전용 도메인 gptour.go.kr(및 www)은 robots.txt가
//   `User-agent:*` / `Disallow:/`로 예외 없이 전면 차단이다(2026-08-01
//   확인). 가평문화원(gpc.or.kr, 검색으로 확인한 실제 도메인)은
//   robots.txt 자체가 404("The requested URL /robots.txt was not found on
//   this server")라 관례대로 비차단으로 판단했지만, "문화행사" 메뉴가
//   "자라섬 정월대보름행사", "석봉 한호선생 전국 휘호대회" 등 프로그램별
//   개별 게시판(`bo_table=saup1_7` 등) 8개로 쪼개져 있고 각 게시판도
//   "2019 자라섬 정월대보름 이모저모", "2016년 2월 20일 토요일 자라섬
//   정월대보름행사"처럼 지난 행사를 회고하는 사진 게시물이며 날짜 필드는
//   작성일(`td_datetime`, 예: "2019-10-18")뿐이라 "등록일만 있는 게시판"
//   기준에 해당하고, 최신 게시물도 2019년에 머물러 있어 실질적으로도
//   미래 일정을 스캔할 수 없어 제외. 가평군 소유의 단일 문화시설
//   음악역1939(music1939.gp.go.kr, robots.txt 자체가 404라 비차단)도
//   확인했으나 "공연 및 행사" 메뉴가 이 시설 자체의 자체 프로그램(THE
//   SHOW/ENSEMBLE/FESTA/TALK CONCERT 등) 안내이지 군 전역의 축제 목록이
//   아니라 스코프 밖으로 판단해 제외. 결과적으로 가평군도 등록 가능한
//   사이트를 찾지 못했다.
//
// - 양평군: 시청 본 도메인(yp21.go.kr)과 관광
//   서브도메인(tour.yp21.go.kr)은 robots.txt가 동일하다 — `User-agent :
//   *`(콜론 앞뒤 공백이 있는 비표준 표기이지만 파서는 통상 공백을
//   허용한다) 그룹이 `Disallow : /`이고, `Googlebot`/`Yeti`/`Daumoa`
//   전용 그룹은 특정 게시판 경로만 `Disallow`한다(2026-08-01 확인). 우리
//   UA는 named-bot 그룹 어디와도 문자열이 일치하지 않아 `*` 그룹이
//   적용되므로 두 도메인 모두 전면 차단이다 — "축제·행사일정" 게시판
//   (`tour.yp21.go.kr/www/selectSchdulWebList.do?key=191`)이 검색으로
//   확인됐지만 robots 차단으로 접근 대상이 아니다(내용 구조는 검증하지
//   않음). 대안인 양평문화재단(ypcf.or.kr)은 robots.txt가 `User-agent: *`
//   / `Allow: /`(가입/로그인/장바구니/관리자 경로만 Disallow)로 전면
//   허용이다(2026-08-01 확인, sitemap.xml도 명시돼 있음). 다만 유일한
//   일정성 페이지인 "문화달력"(`/culture-program`, imweb 빌더 기반)은
//   카드 9개를 확인했는데 카드 목록 어디에도 날짜 텍스트가 전혀 없다
//   (전체 페이지 74만 바이트 중 `YYYY-MM-DD`류 패턴 0건, 2026-08-01
//   확인) — 날짜는 상세 페이지 본문에만 있을 것으로 추정되나 목록에서
//   구조화된 필드로 노출되지 않아 declarative parser로 추출할 수 없다.
//   같은 사이트의 `/festival` 경로는 `/2025-yanggangfestival`("2025
//   양강섬예술축제 소개")로 리다이렉트됐고, `/culture-program`에서 발견한
//   `/2023-festival-dada`, `/2024-festival-dada`,
//   `/2025-yanggangfestival` 같은 URL 패턴도 매년 축제 하나씩을 위해 새로
//   만든 개별 랜딩 페이지들이라 — 이전 wave들의 "단일 행사 프로모션
//   마이크로사이트" 배제 기준과 동일해 제외. 결과적으로 양평군도 시청
//   도메인은 robots 전면 차단, 문화재단의 유일한 일정 게시판은 날짜 필드
//   부재로 제외돼 등록 가능한 사이트를 찾지 못했다.
//
// 결과적으로 wave 22는 4곳 모두 등록 0건이다. 4곳 다 시청 본 도메인(또는
// 그 관광 서브도메인)이 robots.txt의 `*` 그룹 `Disallow: /`로 전면
// 차단됐고(여주만 `/tour/`가 예외), 각 지역에서 새로 찾은 문화재단/문화원/
// 시설관리공단 대안 도메인도 로봇은 통과하는 경우가 많았지만 "등록일만
// 있는 게시판"(가평 2곳), "축제 신호 없이 콘텐츠가 섞인 게시판"(여주),
