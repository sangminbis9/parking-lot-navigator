...
DROP INDEX IF EXISTS idx_local_events_merchant;
...
CREATE TABLE local_events_new (
  ...
  status TEXT NOT NULL DEFAULT 'pending'
    CHECK (status IN ('pending', 'pending_payment', 'approved', 'rejected', 'expired')),
  source TEXT NOT NULL
    CHECK (source IN ('instagram', 'naver_place', 'naver_blog', 'owner_submitted', 'admin_manual', 'user_report', 'official_site', 'merchant', 'other')),
  source_url TEXT,
  source_item_id TEXT,
  image_url TEXT,
...
INSERT INTO local_events_new (
  ...
  raw_payload, rejection_reason, approved_at, merchant_id, payment_key, payment_amount,
...
CREATE INDEX IF NOT EXISTS idx_local_events_merchant
  ON local_events(merchant_id);
